...
describe('local RAG dependency contract', () => {
  ...
  it('keeps only local CPU retrieval packages and stable offline scripts', async () => {
    ...
    expect(manifest.scripts['test:rag:offline']).toBe(
      'vitest run test/unit/rag-chunker.test.ts test/integration/local-e5-encoder.test.ts test/integration/lancedb-rag-index.test.ts test/integration/rag-search-service.test.ts test/evaluation',
    )
  })
})
